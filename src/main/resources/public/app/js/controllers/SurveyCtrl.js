/**
 * Created by Henrikh Kantuni on 4/4/16.
 */
  ...
  function ($scope, $http, RestURL) {
    ...
    self.createSurvey = function () {
      ...
      if (valid) {
        ...
        $http.post(RestURL.baseURL + 'Survey_Default_Activity/create_survey_new', new_survey)
          .success(function (response) {
            console.log(response);
            /* refresh all surveys */
            self.getAllSurveys();

            /* set to undefined create survey fields */
            self.new_survey_name = undefined;
            self.new_survey_description = undefined;
            self.new_survey_country = undefined;
            self.new_survey_company = undefined;
            self.new_survey_product = undefined;
          })
          .error(function (err) {
            console.error(err);
          });
    ...
    self.deleteSurvey = function (survey_id) {
      $http.delete(RestURL.baseURL + 'Survey_Default_Activity/delete_Survey/' + survey_id)
        .success(function (response) {
          /* update surveys after delete */
          self.getAllSurveys();
          console.log(response);
        })
        .error(function (err) {
    ...
    self.getAllSurveys = function () {
      $http.get(RestURL.baseURL + 'Survey_Default_Activity/get_all_survey_new')
        .success(function (response) {
